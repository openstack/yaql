...
class TestCollections(yaql.tests.TestCase):
    ...
    def test_dict_from_sequence(self):
        self.assertEqual(
            {'a': 1, 'b': 2}, self.eval("dict(list(list(a, 1), list('b', 2)))")
        )

        generator = (i for i in (('a', 1), ['b', 2]))
        self.assertEqual(
            {'a': 1, 'b': 2}, self.eval('dict($)', data=generator)
        )

    def test_to_dict(self):
        self.assertEqual(
            {1: 1, 2: 4, 3: 9}, self.eval('$.toDict($, $*$)', data=[1, 2, 3])
        )
    ...
    def test_dict_set_many(self):
        ...
        self.assertEqual(
            ...
            self.eval(
                '$.set(dict(a => 55, "d x" => 99, null => null))', data=data
            ),
        )
        self.assertEqual(data, {'a': 12, 'b c': 44})
    ...
    def test_dict_items(self):
        ...
        self.assertCountEqual(
            [['a', 12], ['b', 44]], self.eval('$.items()', data=data)
        )
    ...
    def test_contains(self):
        ...
        self.assertTrue(
            self.eval('$.values().select(2*$).contains(24)', data=data)
        )
    ...
    def test_list_addition(self):
        self.assertEqual([1, 2, 3, 4], self.eval('list(1, 2) + list(3, 4)'))
        self.assertEqual(
            [1, 2, 6, 8], self.eval('list(1, 2) + list(3, 4).select($ * 2)')
        )

    def test_dict_addition(self):
        self.assertEqual(
            {'a': 1, 'b': 2}, self.eval('dict(a => 1) + dict(b => 2)')
        )
    ...
    def test_insert(self):
        ...
        self.assertEqual(
            [1, ['a', 'b'], 2], self.eval('[1, 2].insert(1, [a, b])')
        )
    ...
    def test_insert_iter(self):
        self.assertEqual(
            [1, 'a', 2], self.eval('[1, 2].select($).insert(1, a)')
        )

        self.assertEqual(
            [1, ['a', 'b'], 2], self.eval('[1, 2].select($).insert(1, [a, b])')
        )

        self.assertEqual([1, 2], self.eval('[1, 2].select($).insert(-1, a)'))

        self.assertEqual(
            [1, 2, 'a'], self.eval('[1, 2].select($).insert(100, a)')
        )

        self.assertEqual(
            ['b', 'a'], self.eval('[].select($).insert(0, a).insert(0, b)')
        )

        self.assertEqual(
            ['a', 'a', 'b'], self.eval('set(a, b).orderBy($).insert(1, a)')
        )

    def test_insert_many(self):
        self.assertEqual(
            [1, 'a', 'b', 2], self.eval('[1, 2].insertMany(1, [a, b])')
        )

        self.assertEqual(
            ['a', 'b', 1, 2], self.eval('[1, 2].insertMany(-1, [a, b])')
        )

        self.assertEqual(
            [1, 2, 'a', 'b'], self.eval('[1, 2].insertMany(100, [a, b])')
        )
    ...
    def test_replace(self):
        self.assertEqual(
            [None, 2, 3, 4], self.eval('[1, 2, 3, 4].replace(0, null)')
        )

        self.assertEqual(
            [None, 3, 4], self.eval('[1, 2, 3, 4].replace(0, null, 2)')
        )
        ...
        self.assertEqual(
            [1, 7, 3, 4], self.eval('set(4, 2, 3, 1).orderBy($).replace(1, 7)')
        )

    def test_replace_many(self):
        self.assertEqual(
            [7, 8, 2, 3, 4], self.eval('[1, 2, 3, 4].replaceMany(0, [7, 8])')
        )

        self.assertEqual(
            [7, 8, 3, 4], self.eval('[1, 2, 3, 4].replaceMany(0, [7, 8], 2)')
        )

        self.assertEqual(
            [1, 7, 8], self.eval('[1, 2, 3, 4].replaceMany(1, [7, 8], -1)')
        )

    def test_delete_dict(self):
        data = {'a': 1, 'b': 2, 'c': 3, 'd': 4}
        self.assertEqual(
            {'a': 1, 'd': 4}, self.eval('$.delete(b, c)', data=data)
        )

    def test_delete_all(self):
        data = {'a': 1, 'b': 2, 'c': 3, 'd': 4}
        self.assertEqual(
            {'a': 1, 'd': 4}, self.eval('$.deleteAll([b, c])', data=data)
        )
    ...
    def test_set_addition(self):
        self.assertCountEqual(
            [4, 3, 2, 1], self.eval('set(1, 2, 3) + set(4, 2, 3)')
        )
    ...
    def test_set_union(self):
        self.assertCountEqual(
            [4, 3, 2, 1], self.eval('set(1, 2, 3).union(set(4, 2, 3))')
        )
    ...
    def test_set_difference(self):
        self.assertCountEqual(
            [4, 1], self.eval('set(1, 2, 3, 4).difference(set(2, 3))')
        )
    ...
    def test_set_add(self):
        ...
        self.assertCountEqual(
            [4, 1, 2, 3, 5], self.eval('set(1, 2, 3).add(4, 5)')
        )

        self.assertCountEqual(
            [1, 3, 2, [1, 2]], self.eval('set(1, 2, 3).add([1, 2])')
        )

        self.assertCountEqual(
            [4, 1, None, 2, 3, 5], self.eval('set(1, 2, 3).add(4, 5, null)')
        )
    ...
    def test_set_remove(self):
        ...
        self.assertCountEqual(
            [3, None], self.eval('set(1, 2, null, 3).remove(1, 2, 5)')
        )

        self.assertCountEqual(
            [3], self.eval('set(1, 2, null, 3).remove(1, 2, 5, null)')
        )

        self.assertCountEqual(
            [1, 3, 2], self.eval('set(1, 2, 3, [1, 2]).remove([1, 2])')
        )
