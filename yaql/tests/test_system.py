...
class TestSystem(yaql.tests.TestCase):
    ...
    def test_unpack_with_names(self):
        ...
        self.assertRaises(
            ValueError, self.eval, '[2, 3].unpack(a, b, c) -> $a + $b'
        )
    ...
    def test_assert(self):
        self.assertEqual(
            [3, 4], self.eval('[2, 3].assert(len($) > 1).select($ + 1)')
        )

        self.assertRaises(
            AssertionError, self.eval, '[2].assert(len($) > 1).select($ + 1)'
        )

        self.assertEqual(
            3, self.eval('[2].select($ + 1).assert(len($) = 1).first()')
        )
    ...
    def test_lambda_func_2nd_order(self):
        self.assertEqual(
            5, self.eval('lambda(let(outer => $) -> lambda($outer - $))(7)(2)')
        )

    def test_lambda_closure(self):
        data = [1, 2, 3, 4, 5, 6]
        self.assertEqual(
            [3, 4, 5, 6], self.eval('$.where(lambda($ > 3)($+1))', data=data)
        )

        # lambda can access value from "where"'s context
        # so we can omit parameter
        self.assertEqual(
            [4, 5, 6], self.eval('$.where(lambda($ > 3)())', data=data)
        )
    ...
    def test_properties(self):
        ...
        self.assertRaises(
            exceptions.NoMatchingFunctionException, self.eval, '"123".negValue'
        )
        self.assertRaises(
            exceptions.NoMatchingFunctionException, self.eval, 'null.negValue'
        )
        self.assertRaises(
    ...
    def test_call_function(self):
        ...
        self.assertRaises(
            exceptions.NoMatchingFunctionException,
            self.eval,
            'call(len, null, {sequence => [1,2]})',
        )
        self.assertRaises(
            exceptions.NoMatchingFunctionException,
            self.eval,
            'call(len, [], {invalid => [1,2]})',
        )
        self.assertRaises(
            exceptions.NoMatchingFunctionException,
            self.eval,
    ...
    def test_call_method(self):
        ...
        self.assertRaises(
            exceptions.NoMatchingFunctionException,
            self.eval,
            'call(len, null, {sequence => [1,2]}, [1, 2])',
        )
        self.assertTrue(self.eval('call(isEmpty, [], {}, null)'))
